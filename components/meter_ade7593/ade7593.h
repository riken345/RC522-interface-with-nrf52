 ...
 * Define Macros
 ************************************************************************/
#define SPI_READ 0b10000000	  // This value tells the ADE7953 that data is to be read from the requested register.
#define SPI_WRITE 0b00000000; // This value tells the ADE7953 that data is to be written to the requested register.
#define SPI_FREQ 1000000	  // Communicate with the ADE7953 at 1 MHz frequency, this is the default value
 ...
 ************************************************************************/
typedef struct spimeterData_st
{
	uint32_t u32Power;
	uint32_t u32Energy;
	uint32_t u32Voltage;
	uint32_t u32Current;
} __attribute__((packed)) spimeterData_st;
/************************************************************************
 * Define Private Variables
 ************************************************************************/

// #define VRMSOS_DEFAULT_VALUE 0x00007ea9
#define VRMSOS_DEFAULT_VALUE 0x0001127d
// #define AIRMSOS_DEFAULT_VALUE 0x000012b1 ////bb44
#define AIRMSOS_DEFAULT_VALUE 0x000006ed
#define AWATTOS_DEFAULT_VALUE 0xfffffff
#define AVAROS_DEFAULT_VALUE 0xffffffff
// #define AVGAIN_DEFAULT_VALUE 0x00027306
#define AVGAIN_DEFAULT_VALUE 0x000d5945

//#define AIGAIN_DEFAULT_VALUE 0x000264EA
#define AIGAIN_DEFAULT_VALUE 0x00026719 


#define AWGAIN_DEFAULT_VALUE 0x02d87878  // 23161 gain
#define AVARGAIN_DEFAULT_VALUE 0x00000002
 ...
 ************************************************************************/
app_err_t spiAlgorithm8_write(uint16_t Address, uint8_t ui32val,
							  uint8_t connector);
app_err_t spiAlgorithm8_read(uint16_t u16Address, uint32_t *ui32val,
							 uint8_t connector);

app_err_t spiAlgorithm32_write(uint16_t Address, uint32_t ui32val,
							   uint8_t connector);
app_err_t spiAlgorithm32_read(uint16_t u16Address, uint32_t *ui32val,
							  uint8_t connector);

app_err_t SPI_MeterWrite(uint16_t u16Address, uint32_t ui32val,
						 uint8_t ByteValue, uint8_t connector);
app_err_t SPI_MeterRead(uint16_t u16Address, uint32_t *ui32val, uint8_t ByteValue, uint8_t connector);
...
app_err_t getIrmsA(uint8_t);
app_err_t getVrms(uint8_t);
app_err_t getInstReactivePowerA(uint8_t);
app_err_t getActiveEnergyA(uint8_t);
app_err_t getVersion(uint8_t *pui8Val, uint8_t connnector);
...
#if 0
void ADE7953_spiAlgorithm8_write(uint8_t MSB, uint8_t LSB, uint8_t onemsb) ;
uint32_t ADE7953_spiAlgorithm32_read(uint8_t MSB, uint8_t LSB);
void ADE7953_spiAlgorithm32_write(uint8_t MSB, uint8_t LSB, uint8_t onemsb, uint8_t two, uint8_t three, uint8_t fourlsb);
float ADE7953_getVrms();
float ADE7953_getIrmsA();
 unsigned long getAPNOLOAD(void);
 uint8_t functionBitVal(int addr, uint8_t byteVal);
#endif
/****************************************************************************
 * END OF FILE
 ****************************************************************************/
